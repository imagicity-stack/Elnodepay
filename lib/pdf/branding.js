...
export const createBrandedPdf = (jsPDFConstructor) => {
  ...
  const drawBranding = () => {
    ...
    doc.roundedRect(
      PDF_BRAND.sideMargin / 2,
      PDF_BRAND.headerHeight - 3,
      pageWidth - PDF_BRAND.sideMargin,
      6,
      2,
      2,
      'FD',
...
export const drawSectionHeading = (doc, bounds, title, subtitle = '') => {
  ...
  doc.setDrawColor(PDF_BRAND.strokeMuted);
  doc.roundedRect(bounds.left - 2, bounds.startY - 4, bounds.width + 4, bandHeight + (subtitle ? 6 : 2), 2, 2, 'FD');

  doc.setFont(PDF_BRAND.headerFont, 'bold');
  doc.setFontSize(12);
  doc.setTextColor(20, 20, 20);
  doc.text(title, bounds.left + 2, bounds.startY + 3);

  if (subtitle) {
    doc.setFont(PDF_BRAND.headerFont, 'normal');
    doc.setFontSize(10);
    doc.setTextColor(73, 80, 87);
    doc.text(subtitle, bounds.left + 2, bounds.startY + 9);
  }
