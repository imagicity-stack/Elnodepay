...
export const createFeeCollectionReportPdf = (
  ...
  const columns = [
    { key: 'label', label: 'Payment Number', width: 0.14 },
    { key: 'amount', label: 'Amount', width: 0.14 },
    { key: 'mode', label: 'Mode', width: 0.12 },
    { key: 'dates', label: 'Date and Time', width: 0.24 },
    { key: 'transaction', label: 'Transaction ID', width: 0.16 },
  ...
  const leftX = bounds.left + 3;
  const rightX = bounds.left + bounds.width / 2 + 3;
  doc.setFontSize(10);
  infoPairs.slice(0, 3).forEach(([label, value], index) => {
    const offsetY = y + 4 + index * 8;
    doc.setFont(PDF_BRAND.headerFont, 'bold');
    doc.text(`${label}:`, leftX, offsetY);
    doc.setFont(PDF_BRAND.headerFont, 'normal');
    doc.text(value, leftX + 32, offsetY);
  });
  infoPairs.slice(3).forEach(([label, value], index) => {
    ...
    doc.setFont(PDF_BRAND.headerFont, 'normal');
    doc.text(value, rightX + 36, offsetY);
  });
  ...
  const renderRow = (entry, index) => {
    ...
    const textHeights = cellLines
      ...
      .map((lines) => Math.max(12, lines.length * 5 + 6));
    const breakdownHeight = Math.max(12, Math.max(1, breakdownItems.length) * 6 + 6);
    const rowHeight = Math.max(...textHeights, breakdownHeight, 16);
      ...
      if (col.key === 'breakdown') {
        ...
        if (!breakdownItems.length) {
          ...
          breakdownItems.forEach((item) => {
            ...
            doc.setFont(PDF_BRAND.headerFont, 'normal');
            doc.text(item.amount, x + colWidth - 2, lineY, { align: 'right' });
            lineY += 5.2;
          });
        }
      } else {
        doc.text(cellLines[colIndex], x + 2, y + 4.5, { baseline: 'top' });
      }
      x += colWidth;
    });
