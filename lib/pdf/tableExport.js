...
export const createTabularPdf = (jsPDFConstructor, { title, headers = [], rows = [] }) => {
  ...
  const drawTableHeader = () => {
    ...
    headers.forEach((header, index) => {
      ...
      const labelLines = doc.splitTextToSize(header.label, columnWidth - 6);
  ...
  const renderRow = (rowData) => {
    ...
    const rowHeight = Math.max(...cellLines.map((lines) => lines.length)) * 5 + 6;
    ...
    headers.forEach((header, index) => {
      ...
      doc.setFontSize(9.5);
      doc.setTextColor(PDF_BRAND.textColor);
      doc.text(cellLines[index], x + 3, y + 5.5);
    });
    y += rowHeight;
  };
