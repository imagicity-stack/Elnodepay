...
const buildInquiryQuery = (filters = {}, searchTerm = '') => {
  ...
  if (filters.followUpDueToday) {
    ...
    constraints.push(where('followUpDate', '>=', start));
    constraints.push(where('followUpDate', '<=', end));
  }
  if (filters.dateFrom && filters.dateTo) {
    constraints.push(where('createdAt', '>=', filters.dateFrom));
    constraints.push(where('createdAt', '<=', filters.dateTo));
  }
  let q = query(collection(db, INQUIRIES_COLLECTION), ...constraints, orderBy('createdAt', 'desc'));
  if (searchTerm) {
    q = query(collection(db, INQUIRIES_COLLECTION), ...constraints, orderBy('parentName')); // sorted for client filtering
  }
  return q;
};
