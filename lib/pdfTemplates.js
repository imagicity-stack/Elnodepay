...
let currencySymbol = '₹';
const formatCurrency = (value = 0, symbol = currencySymbol) =>
  `${symbol}${Number(value || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`;
const BRAND_FONT_URLS = {
  regular: '/fonts/NotoSerif-Regular.ttf',
  bold: '/fonts/NotoSerif-Bold.ttf',
};
let brandFontBytesCache = null;
...
const getBrandFonts = async () => {
  if (!brandFontBytesCache) {
    brandFontBytesCache = {
      regular: await fetchFontBytes(BRAND_FONT_URLS.regular),
      bold: await fetchFontBytes(BRAND_FONT_URLS.bold),
    };
  }
  return brandFontBytesCache;
};
...
const createDocument = async () => {
  ...
  const fonts = await (async () => {
    try {
      const brandFontBytes = await getBrandFonts();
      currencySymbol = '₹';
      return {
        regular: await pdfDoc.embedFont(brandFontBytes.regular, { subset: true }),
        bold: await pdfDoc.embedFont(brandFontBytes.bold, { subset: true }),
      };
    } catch (error) {
      console.error('Falling back to standard fonts for PDF generation', error);
      currencySymbol = 'Rs. ';
      return {
